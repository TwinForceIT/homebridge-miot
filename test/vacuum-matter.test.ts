  ...
  async request(method: string, params: MiotParams): Promise<unknown> {
    ...
    if (method === 'action') {
      ...
      if (!this.rejectActions && !this.ignoreActions) {
        this.values.set('2/1', action.siid === 3 ? 3 : action.aiid === 2 ? 2 : 5);
      }
      return { ...action, code: this.rejectActions ? -4001 : 0, out: [] };
    }
    assert.ok(Array.isArray(params));
    ...
    if (method === 'set_properties') {
      ...
      this.values.set(`${property.siid}/${property.piid}`, property.value!);
...
function fixture() {
  ...
  const logger = { warn: (message: string) => warnings.push(message), info: (message: string) => logs.push(message) } as unknown as Logger;
  ...
  return { emulator, warnings, logs, updates, controller, accessory, state, command, matter };
}
